"""
## Communicating messages with the UI

During ArC2Control startup a set of [Qt signals][1] will be initialised. These are
used to pass messages between the UI and various constituent components as well
as internal and external modules. Current set of signals includes arc2 status
changes (connection and configuration) as well as data updates to be propagated
to the UI. Modules can connect and trigger all modules here although
practically connection and configuration signals need not be triggered
explicitly. If you are subclassing `arc2control.modules.base.BaseModule` then
all the required signals are preconnected and the values they correspond to are
exposed directly as object properties or methods. You can still connect to them
but it's not recommended to emit them explicitly. Read on for further details.

  [1]: https://www.riverbankcomputing.com/static/Docs/PyQt6/signals_slots.html
"""
...
selected a different set of devices. Please not that this is *not* triggered
when the same devices have been selected.

*It's not recommended to emit this signal from modules, unless you want to
change the global read-out voltage (which is also not recommended).*
