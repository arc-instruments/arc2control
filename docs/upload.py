...
from arc2control.version import __version__ as ARC2VERSION
...
def docs_version():
    ...
    lines = open(join_paths(__HERE__, 'conf.py')).readlines()
...
def find_local_version():
    ...
    pyproject_tool = tomli.loads(open(join_paths(__ABOVE__, 'pyproject.toml')).read())['tool']['poetry']['version']
    docs = docs_version()

    versions = [ARC2VERSION, pyproject_tool, docs]
    print(versions, file=sys.stderr)

    # Check if the same version is used throughout
    consistent = all(v == versions[0] for v in versions)
    if len(versions) < 3:
        raise ValueError('Not all of ARC2VERSION, pyproject.toml or docs/conf.py '
            'define versions')
    elif not consistent:
        # complain if it doesn't
        raise ValueError('ARC2VERSION.toml, pyproject.toml and docs/conf.py '
...
def main(project, host, username, password):
    ...
    version = gitv
    basedir = join_paths(__HERE__, '_build', 'html')
    ...
    with ftputil.FTPHost(host, username, password) as ftp:
        ...
        for (root, dirs, files) in os.walk(basedir):
            for f in files:
                source = join_paths(root, f)
                path = relpath(source, basedir)
